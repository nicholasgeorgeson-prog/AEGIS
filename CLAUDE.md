...
### 169. Frontend Timeout Must Exceed Backend Auth Time (v6.3.0)
**Problem**: SharePoint scan greyed out then showed "Retry Scan" after 30 seconds. User reported: "I hit scan all it greys out then nothing now I see retry scan."
**Root Cause**: TWO compounding issues:
1. **Server not restarted after v6.2.9 deploy**: The v6.2.9 apply script downloaded new Python files (review_routes.py with async background thread pattern), but the AEGIS server was NOT restarted. Python backend changes require server restart (Lesson 4). The old synchronous code was still in memory — `sharepoint_scan_selected()` created HeadlessSPConnector inline, blocking for 30+ seconds.
2. **30-second AbortController too tight**: The frontend `_startSPSelectedScan()` had a 30-second timeout. HeadlessSP auth (Playwright launch → Edge → SSO redirect chain → verify) takes 15-45 seconds. Even when the backend worked correctly, the timeout was barely sufficient.
**Evidence**: sharepoint.log showed server started at 12:00:56, no restart until 18:03:54. The scan attempt at 17:07 (from console_logs.json) was running OLD code. CSRF token was obtained in 34ms (proving Flask was responsive), but the scan POST hung for exactly 30 seconds before being aborted.
