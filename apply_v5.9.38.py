...
Usage:
    ...
    python apply_v5.9.38.py
    python3 apply_v5.9.38.py
...
FILES = [
    # SharePoint connector (improved error diagnostics, connect_and_discover)
    "sharepoint_connector.py",
    "version.json",
    "static/version.json",

    # Routes (new connect-and-scan endpoint, batch_mode for SP scans)
    "routes/review_routes.py",

    # Templates (updated SP section with Connect & Scan button)
    "templates/index.html",

    # JavaScript (Connect & Scan handler)
    "static/js/app.js",

    # Help docs (version history update)
...
def main():
    ...
    print()
    print("  =============================================")
    print("    AEGIS v5.9.38 Direct Updater")
    print("    SharePoint Scan Improvements")
    print("  =============================================")
    print()
    print("  SHAREPOINT FIXES:")
    print("    - Better error diagnostics (SSL/DNS/proxy)")
    print("    - Increased connection timeout (30s→45s)")
    print("    - Auto-tries SSL bypass on all errors")
    print("    - Fresh SSO session on retry")
    print()
    print("  SHAREPOINT UX:")
    print("    - One-click 'Connect & Scan' button")
    print("    - Library path auto-populates from URL")
    print("    - No manual folder entry required")
    print("    - SharePoint scans use batch_mode")
    print()
    ...
    backup_dir = os.path.join(install_dir, "backups", f"pre_v5.9.38_{timestamp}")
    ...
    if failed == 0:
        ...
        print("    3. Try the SharePoint scan — one-click 'Connect & Scan'!")
        print()
