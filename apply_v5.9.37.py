...
Usage:
    ...
    python apply_v5.9.37.py
    python3 apply_v5.9.37.py
...
FILES = [
    # Python backend (core.py has persistent Docling worker pool)
    "core.py",
    "version.json",
    "static/version.json",

    # Routes (batch_mode option, increased workers/timeouts)
...
def main():
    ...
    print()
    print("  =============================================")
    print("    AEGIS v5.9.37 Direct Updater")
    print("    Batch Scan Performance Optimization")
    print("  =============================================")
    print()
    print("  PERFORMANCE:")
    print("    - Persistent Docling worker pool (no more")
    print("      15-30s startup overhead per document)")
    print("    - batch_mode skips html_preview during batch")
    print("    - Worker pool 3→4, chunk size 5→8")
    print("    - Per-file timeout 5min→8min")
    print("    - Expected: 3-6x faster batch scans")
    print()
    print("  ZERO ACCURACY LOSS:")
    print("    - Docling extraction quality identical")
    print("    - All checkers run same as before")
    print("    - Single-doc scans completely unchanged")
    print()
    ...
    backup_dir = os.path.join(install_dir, "backups", f"pre_v5.9.37_{timestamp}")
    ...
    if failed == 0:
        ...
        print("    3. Run a batch scan — should be 3-6x faster!")
        print()
