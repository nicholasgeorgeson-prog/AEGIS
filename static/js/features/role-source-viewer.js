...
TWR.RoleSourceViewer = (function() {
    ...
    async function fetchRoleDataFromAPI(roleName) {
        try {
            ...
            if (response.ok) {
                ...
                if (data && data.role_name && data.occurrences && data.occurrences.length > 0) {
    ...
    async function open(roleName, options) {
        ...
        // Update loading UI
        State.modal.querySelector('.rsv-role-name').textContent = roleName;
        ...
        State.modal.querySelector('.rsv-context-status').textContent = '';
        ...
        if (!roleData || roleData.documents.length === 0) {
            ...
            State.modal.querySelector('.rsv-context-status').innerHTML = `
