...
window.HyperlinkValidator = (function() {
    ...
    function renderExcelSummary(data) {
        // v5.9.1: Count by detailed validation status (matching renderSummary breakdown)
        const counts = { working: 0, broken: 0, redirect: 0, timeout: 0, blocked: 0, auth_required: 0, unknown: 0 };
        let mailto = 0;
        ...
        data.links.forEach(link => {
            ...
            else if (s === 'AUTH_REQUIRED') counts.auth_required++;
            else counts.unknown++;
        });
        ...
        const statIds = {
            ...
            'hv-count-auth': counts.auth_required,
    ...
    function updateCapabilitiesDisplay() {
        ...
        // v5.9.1: Show warning when Windows Auth is selected but not available
        const authWarning = document.getElementById('hv-auth-warning');
        if (authWarning) {
            const winAuthAvailable = caps.authentication?.windows_sso?.available ||
                                     caps.features?.windows_sso;
            const isValidatorMode = el.modeSelect?.value === 'validator';
            authWarning.style.display = (isValidatorMode && !winAuthAvailable) ? 'flex' : 'none';

            // Also update on mode change
            if (el.modeSelect && !el.modeSelect._authWarnListener) {
                el.modeSelect.addEventListener('change', () => {
                    const isVal = el.modeSelect.value === 'validator';
                    authWarning.style.display = (isVal && !winAuthAvailable) ? 'flex' : 'none';
                });
                el.modeSelect._authWarnListener = true;
            }

            // Refresh Lucide icons for the warning
